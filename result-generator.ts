...
import { DIMENSION_KEYS, DimensionKey, DimensionScores, QuizResult } from "@/types/quiz";
...
function decodePayload(value: string): SerializedResult | null {
  try {
    ...
    if (
      ...
      parsed === null ||
      typeof (parsed as SerializedResult).typeCode !== "string" ||
      typeof (parsed as SerializedResult).matchPercentage !== "number" ||
      typeof (parsed as SerializedResult).scores !== "object" ||
      (parsed as SerializedResult).scores === null
    ) {
      return null;
    }

    const scores = (parsed as SerializedResult).scores;
    const hasValidScores = DIMENSION_KEYS.every(
      (key) => typeof scores[key] === "number" && Number.isFinite(scores[key]),
    );

    return hasValidScores ? (parsed as SerializedResult) : null;
  } catch {
    return null;
  }
}
